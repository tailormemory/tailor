"""
rebuild_db_v2.py — Ricostruisce la collection tailor_kb_v2 da zero.

Variante v2 di rebuild_db.py (originale: rebuild v1 tailor_kb del 25/04).
Cambi vs. originale:
  - SQLITE_PATH: chroma.sqlite3 (live), non .corrupted → MCP DEVE essere
    DOWN durante il run (single-writer). Switchover finale via UPDATE
    diretto sulla stessa sqlite: il bug switchover dell'originale
    (UPDATE su .corrupted invece che sul live) si autorisolve perché
    client + UPDATE operano sullo stesso file.
  - OLD_COLLECTION/NEW_COLLECTION: v2 → v2_new (post-switch torna v2)
  - OLD_METADATA_SEGMENT: 56a260e7-... (segment metadata v2)

Use case 2026-05-28: drenare WAL stuck di 780+ ops, riparare FTS5
malformed inverted index, sostituire pickle legacy-dict con formato
1.5.9-current.

Uso:
    cd /path/to/tailor
    .venv/bin/python3 scripts/maintenance/rebuild_db_v2.py --dry-run
    .venv/bin/python3 scripts/maintenance/rebuild_db_v2.py            # MCP DOWN
    .venv/bin/python3 scripts/maintenance/rebuild_db_v2.py --resume

Tempo stimato: ~80-95 minuti per ~159k chunk (Ollama locale nomic-embed).
"""
...
OLD_COLLECTION = "tailor_kb_v2"
NEW_COLLECTION = "tailor_kb_v2_new"

# Segment ID del METADATA segment di tailor_kb_v2
# (filter to read only chunks from the v2 collection, not v1 or v2_new)
OLD_METADATA_SEGMENT = "56a260e7-1843-4667-8d8b-5d79a0223d51"
...
def main():
    ...
    print()
    print("Prossimi step:")
    print("  1. Verifica: .venv/bin/python3 -c \"import chromadb; c=chromadb.PersistentClient(path='db'); col=c.get_collection('tailor_kb_v2'); print(col.count())\"")
    print("  2. Riavvia MCP: sudo launchctl bootstrap system /Library/LaunchDaemons/com.tailor.mcp.plist")
